...
def sell():
    if request.method == "POST":
        ...
        current_shares = db.execute("SELECE shares FROM orders WHERE user_id = ? AND symbol = ? GROUP BY symbol", user_id, symbol)[0]["shares"]
        ...
        db.execute("INSERT INTO orders (user_id, name, price, type, symbol) VALUES (?, ?, ?, ?, ?, ?)", user_id, name, -shares, price, "sell", symbol)
        return redirect("/")
