...
#db.execute("CREATE TABLE IF NOT EXISTS orders (id INTEGER, user_id NUMERIC NOT NULL, symbol TEXT NOT NULL, shares NUMERIC NOT NULL, price NUMERIC NOT NULL, timestamp TEXT, name TEXT, type TEXT, PRIMARY KEY(id), FOREIGN KEY(user_id) REFERENCES users(id))")
...
def buy():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            return apology("please enter an int")
...
def sell():
    if request.method == "POST":
        ...
        current_shares = db.execute("SELECT shares FROM orders WHERE user_id = ? AND symbol = ? GROUP BY symbol", user_id, symbol)[0]["shares"]
        ...
        db.execute("INSERT INTO orders (user_id, shares, price, type, symbol) VALUES (?, ?, ?, ?, ?)", user_id, -shares, price, "sell", symbol)
        return redirect("/")
